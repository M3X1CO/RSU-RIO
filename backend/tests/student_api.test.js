...
describe('When there is initially some students saved', () => {
  ...
  test('The first student name is...', async () => {
    ...
    assert.strictEqual(names.includes('Tonyis'), true)
  })
  ...
  describe('when there is initially one user at db', () => {
    beforeEach(async () => {
      ...
      const passwordHash = await bcrypt.hash('sekret', 10)
      const user = new User({ username: 'root06', passwordHash })
      await user.save()
    ...
    test('creation fails with proper statuscode and message if username or password is too short', async () => {
      ...
      assert(result.body.error.includes('Username and password must be at least 6 characters long'))
